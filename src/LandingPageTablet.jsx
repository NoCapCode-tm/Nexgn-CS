...
zIndex: 0,
maskImage: "linear-gradient(to left, rgba(0,0,0,1) 40%, rgba(0,0,0,0) 100%) linear-gradient(to top, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 15%)",
WebkitMaskImage: "linear-gradient(to left, rgba(0,0,0,1) 40%, rgba(0,0,0,0) 100%) linear-gradient(to top, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 15%)",
      ...
      <div style={{ position: "relative", display: "flex", gap: 16, maxWidth: 900, margin: "0 auto", alignItems: "flex-start" }}>
        ...
        {features.map(f => (
          <div key={f.title} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative", zIndex: 1 }}>
            <div style={{ width: 72, height: 72, background: "#fff", borderRadius: 18, boxShadow: "0 4px 20px rgba(0,0,0,0.10)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
              {f.icon}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, maxWidth: 900, margin: "36px auto 0" }}>
        {features.map(f => (
          <div key={f.title} style={{
            ...
            <p style={{ fontSize: 12, color: "#777", lineHeight: 1.6, fontFamily: "'Inter', sans-serif" }}>{f.desc}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section style={{ padding: "64px 32px", background: "#F9F5F7", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 16, maxWidth: 900, margin: "0 auto" }}>
        {units.map(([label, val]) => (
          <div key={label} style={{ width: 162, height: 186, position: "relative", filter: "drop-shadow(0px 4px 13.1px rgba(255,9,21,0.16))" }}>
            <div style={{ position: "absolute", inset: 0, borderRadius: 13, background: "rgba(255,222,222,0.12)" }}/>
            <div style={{ position: "absolute", top: 8, left: 10, right: 10, height: 82, borderRadius: 10, background: "#FEF7F6", boxShadow: "0px 2px 9.8px rgba(255,157,166,0.25)", overflow: "hidden", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
              <span style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 68, lineHeight: 1, color: BRAND, transform: "translateY(34px)" }}>{String(val).padStart(2, "0")}</span>
            </div>
            <div style={{ position: "absolute", top: 98, left: 10, right: 10, bottom: 8, borderRadius: 10, background: "#FEF7F6", boxShadow: "0px 0px 9.8px rgba(255,157,166,0.18)", overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start" }}>
              <span style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 68, lineHeight: 1, color: BRAND, transform: "translateY(-34px)" }}>{String(val).padStart(2, "0")}</span>
              <span style={{ position: "absolute", bottom: 8, fontSize: 10, fontWeight: 500, letterSpacing: "2px", textTransform: "uppercase", color: "#aaa", fontFamily: "'Inter', sans-serif" }}>{label}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: "flex", gap: 16, flexShrink: 0 }}>
          {stats.map(s => (
            <div key={s.title} style={{ textAlign: "center" }}>
              ...
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 12, color: "#1a0a0d" }}>{s.title}</div>
              <div style={{ fontSize: 11, color: "#aaa", fontFamily: "'Inter', sans-serif", marginTop: 2 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
